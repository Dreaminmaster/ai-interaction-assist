...
app.get('/health', (_req, res) => {
  res.json({ ok: true, service: 'ai-interaction-assist-local-app-learning-workbench-maps' });
});
...
app.get('/', (_req, res) => {
  res.sendFile(path.join(publicDir, 'index.learning-workbench.dynamic.html'));
});
...
app.listen(config.port, async () => {
  ...
  console.log(`Learning workbench with maps listening at ${url}`);
