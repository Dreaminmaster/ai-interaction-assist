...
app.get('/health', (_req, res) => {
  res.json({ ok: true, service: 'ai-interaction-assist-local-app-outline-workbench-v2' });
});
...
app.get('/', (_req, res) => {
  res.sendFile(path.join(publicDir, 'index.outline-workbench.v2.html'));
});
...
app.listen(config.port, async () => {
  ...
  console.log(`Outline workbench v2 listening at ${url}`);
