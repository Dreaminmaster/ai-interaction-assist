...
app.get('/health', (_req, res) => {
  res.json({ ok: true, service: 'ai-interaction-assist-local-app-one-click-backup' });
});
...
app.get('/', (_req, res) => {
  res.sendFile(path.join(publicDir, 'index.file-state.html'));
});
...
app.listen(config.port, async () => {
  ...
  console.log(`One-click local app with backup listening at ${url}`);
