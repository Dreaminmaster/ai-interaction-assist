...
app.get('/health', (_req, res) => {
  res.json({ ok: true, service: 'ai-interaction-assist-local-app-outline-workbench' });
});
...
app.get('/', (_req, res) => {
  res.sendFile(path.join(publicDir, 'index.outline-workbench.html'));
});
...
app.listen(config.port, async () => {
  ...
  console.log(`Outline workbench listening at ${url}`);
