...
import { learningMapRouter } from './learningMapRoutes';
...
app.use('/api', learningMapRouter);
app.use('/api', followUpRouter);

app.get('/health', (_req, res) => {
  res.json({ ok: true, service: 'ai-interaction-assist-local-app-learning-workbench-full-v2' });
});
...
app.get('/', (_req, res) => {
  res.sendFile(path.join(publicDir, 'index.learning-workbench.full.v2.html'));
});
...
app.listen(config.port, async () => {
  ...
  console.log(`Full v2 learning workbench listening at ${url}`);
