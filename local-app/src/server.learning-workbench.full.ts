...
app.get('/health', (_req, res) => {
  res.json({ ok: true, service: 'ai-interaction-assist-local-app-learning-workbench-full' });
});
...
app.get('/', (_req, res) => {
  res.sendFile(path.join(publicDir, 'index.learning-workbench.full.html'));
});
...
app.listen(config.port, async () => {
  ...
  console.log(`Full learning workbench listening at ${url}`);
