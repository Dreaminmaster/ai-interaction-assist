...
app.get('/health', (_req, res) => {
  res.json({ ok: true, service: 'ai-interaction-assist-local-app-interactive-providers' });
});
...
app.get('/', (_req, res) => {
  res.sendFile(path.join(publicDir, 'index.providers-expanded.html'));
});
...
app.listen(config.port, async () => {
  ...
  console.log(`Interactive providers local app listening at ${url}`);
