...
app.get('/health', (_req, res) => {
  res.json({ ok: true, service: 'ai-interaction-assist-local-app-outline-workbench-v3' });
});
...
app.get('/', (_req, res) => {
  res.sendFile(path.join(publicDir, 'index.outline-workbench.v3.html'));
});
...
app.listen(config.port, async () => {
  ...
  console.log(`Outline workbench v3 listening at ${url}`);
