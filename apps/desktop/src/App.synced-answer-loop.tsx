...
import { applyAnswerToSession } from './lib/analysis';
import type { LearningMode } from './types';

const modes: LearningMode[] = ['补基础模式', '理解模式', '考试模式', '答辩模式', '论文模式', '项目模式'];

function AppSyncedAnswerLoop() {
  ...
  return (
    <div className="app-shell">
      ...
      <main className="center-panel">
        ...
        <AnswerInputPanel disabled={!activeSession} onSubmit={handleAnswerSubmit} />
      ...
      <aside className="right-panel">
        ...
        <div className="chat-card assistant">
          ...
          <div className="chat-text">当前页面已进入“主题同步闭环版”。提交回答后，不仅 session 会更新，topic 档案也会同步刷新。</div>
        </div>
        <div className="chat-card assistant">
          <div className="chat-role">下一步</div>
          <div className="chat-text">后续最值得补的是：用真正的 AI 分析替换规则分析，并在回答后自动生成下一层学习内容。</div>
        </div>
      </aside>
    </div>
  );
...
export default AppSyncedAnswerLoop;
