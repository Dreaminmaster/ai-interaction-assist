...
import { analyzeTopicAnswer } from './ai/analyzeTopicAnswer';
...
function AppAiAnswerLoop() {
  ...
  async function handleAnswerSubmit(answer: string) {
    ...
    setIsAnalyzing(true);
    try {
      const nextSession = await analyzeTopicAnswer({
        topic: selectedTopic,
    ...
    <div className="app-shell">
      ...
      <aside className="right-panel">
        ...
        <div className="chat-card assistant">
          ...
          <div className="chat-text">当前页面已切到“AI 分析接口版”。现在虽然默认还是 mock provider，但分析服务已经抽象成可替换接口。</div>
        </div>
        <div className="chat-card assistant">
          <div className="chat-role">状态</div>
          <div className="chat-text">{isAnalyzing ? '正在分析当前回答…' : '当前空闲，可继续开始本轮学习或提交回答。'}</div>
        </div>
      </aside>
    </div>
  );
...
export default AppAiAnswerLoop;
