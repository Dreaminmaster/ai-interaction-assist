...
import { syncTopicsWithSession } from '../lib/topic-sync';
import type { LearningMode, SessionRecord, Topic } from '../types';

export function useSyncedTopics() {
  ...
  function applySessionToTopics(session: SessionRecord) {
    const nextTopics = syncTopicsWithSession(topics, session);
    setTopics(nextTopics);
    saveTopics(nextTopics);
  }
